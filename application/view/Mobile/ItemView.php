<?php
class Mobile_ItemView extends Project_View_Block_Abstract
{
	...
	{
		$info = Factory::getResource('mobile')->showInfo($id);
		if ($info) {
			if ($info['gallery']) {
			...
			if ($info['data']['brand'] != '15') {
				$models = Factory::getView('system/mlib')->loadModelListById($info['data']['brand']);
	...
	{
		$info = Factory::getResource('mobile')->showContact($id);
		if ($info) {
	 ...
	 * @return false或咨询列表
	 */
	public function loadMobileConsult($id, $page = 1, $per = 10)
	{
	 ...
	 * @return false或咨询总量
	 */
	public function getMobileConsultTotal($id)
	{
	...
	{
		return Factory::getResource('mobile')->getLastRefreshedAt($id);
	}
	...
	{
		return Factory::getResource('relation/mobile/filter')->getMobileModelId($id);
	}
}
