<?php
class Resource_Store_Mobile_Model extends Project_Model_DB_Abstract
{
	public function __construct()
	{
		parent::__construct();
		$this->_table = 'store_mobile_model';
	}
	 ...
	 * @param 显示数量 $amount
	 */
	public function getSellingModelList($id, $amount = 6)
	{
		$sql = 'select a.id, b.label as brand, c.label as model, c.thumb 
				from store_mobile_model as a 
				left join mlib_brand as b on b.id = a.brand_id 
				left join mlib_model as c on c.id = a.model_id 
	 ...
	 * @param 单页数量 $per
	 */
	public function loadSellingMobileModelList($id, $filter, $sort, $page = 1, $per = 9)
	{
		$where = '';
		$params = array(':id' => $id);
		...
		$sql = 'select a.id, b.label as brand, c.label as model, c.thumb, max(d.price) as max, min(d.price) as min 
				...
				left join mlib_brand as b on b.id = a.brand_id 
				left join mlib_model as c on c.id = a.model_id 
				left join store_mobile_item as d on d.store_model_id = a.id 
	...
	{
		$sql = 'select a.brand_id, b.label as brand, a.id as store_model_id, c.label as model, a.status as model_status, d.id as store_mobile_id, d.version, d.color, d.storage, d.isnew, d.newly, d.remark, d.price, d.status as mobile_status, a.hot, a.recommend 
				from store_mobile_model as a 
				left join mlib_brand as b on b.id = a.brand_id 
				left join mlib_model as c on c.id = a.model_id 
				left join store_mobile_item as d on d.store_model_id = a.id 
	...
	{
		$sql = 'select a.brand_id, b.label as brand, a.id as store_model_id, c.label as model, d.id as store_mobile_id, d.version, d.color, d.storage, d.isnew, d.newly, d.remark, d.price 
				from store_mobile_model as a 
				left join mlib_brand as b on b.id = a.brand_id 
				left join mlib_model as c on c.id = a.model_id 
				left join store_mobile_item as d on d.store_model_id = a.id 
	...
	{
		$sql = 'select model_id from store_mobile_model where id = :id limit 1';
		$select = $this->select($sql, array(':id' => $id));
		return $this->fetchOne($select);
	}
	...
	{
		$sql = 'select count(id) from store_mobile_model where store_id = :store and hot = 1';
	...
	public function hasSellingMobile($id)
	{
		$sql = 'select a.id 
				from store_mobile_model as a 
				left join store_mobile_item as b on b.store_model_id = a.id 
