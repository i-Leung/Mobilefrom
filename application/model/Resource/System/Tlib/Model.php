<?php
class Resource_System_Tlib_Model extends Project_Model_DB_Abstract
{
	public function __construct()
	{
		parent::__construct();
		$this->_table = 'tlib_model';
	}
	...
	public function loadModelList($filter, $order, $page = 1, $per = 16)//, $available = '-1'
	{
		...
		if (isset($filter['os'])) {
			$where .= ' a.id in (
							select model_id from tlib_model_attribute_index 
							where attribute_id = 5 and value_id = :os 
						) and ';
			$params[':os'] = $filter['os'];
		}
		//排序
		...
		$sql = 'select a.id, a.label as model, b.label as brand, a.thumb, a.clicks, a.offers 
				from `tlib_model` as a 
				left join `tlib_brand` as b on b.id = a.bid 
	...
	public function getModelTotal($filter)
	{
		...
		$sql = 'select count(id) from `tlib_model` 
	...
	{
		$sql = 'select params from `tlib_model` where id = :id limit 1';
		$select = $this->select($sql, array(':id' => $id));
		return $this->fetchOne($select);
	}
	...
	public function loadModelQuote($data, $isnew = '1')
	{
		switch ($isnew) {
			case '1':
				$sql = 'select b.model_id as id, min(a.price) as price from store_tablet_item as a 
						left join store_tablet_model as b force index (model) on b.id = a.store_model_id 
						where b.model_id in (' . $data . ') and a.isnew = 1 and a.status = 1 
						group by b.model_id';
				break;
			case '0':
				$sql = 'select id, min(price) as price from (
							select b.model_id as id, min(a.price) as price from store_tablet_item as a 
							left join store_tablet_model as b force index (model) on b.id = a.store_model_id 
							where b.model_id in (' . $data . ') and a.isnew = 0 and a.status = 1 
							group by b.model_id 
							union 
							select b.value_id as id, min(a.price) as price from tablet as a 
							left join relation_tablet_filter_value as b on b.tablet_id = a.id 
							where b.filter_id = 5 and value_id in (' . $data . ') 
							group by b.value_id 
						) as tmp 
						group by id';
				break;
	...
	public function getModelQuote($id, $isnew = '1')
	{
		switch ($isnew) {
			case '1':
				$sql = 'select min(a.price) as price from store_tablet_item as a 
						left join store_tablet_model as b force index (model) on b.id = a.store_model_id 
			...
			case '0':
				$sql = 'select min(price) as price from (
							...
							select b.value_id as id, min(a.price) as price from tablet as a 
							left join relation_tablet_filter_value as b on b.tablet_id = a.id 
							where b.filter_id = 5 and value_id = :value 
							group by b.value_id 
						) as tmp 
						group by id';
	...
	public function loadInsideRecords($store, $q)
	{
		$sql = 'select b.label as brand, a.label as model, a.thumb, d.store_model_id as id, max(d.price) as `max`, min(d.price) as `min` 
				from tlib_model as a 
				left join tlib_brand as b on b.id = a.bid 
				left join store_tablet_model as c on c.model_id = a.id 
				left join store_tablet_item as d on d.store_model_id = c.id 
	...
	public function loadAdminModelList($bid, $sort = 'id')
	{
		...
		$sql = 'select id, label, sort, thumb, status from `tlib_model` where bid = :bid and status = 1 order by ' . $sort;
		$select = $this->select($sql, array(':bid' => $bid));
		return $this->fetchAll($select);
	}
