...
function addMajorImage(src) {
	...
	var img = '<img style="margin:5px;border:solid 1px #ddd;" src="/upload/tlib/thumb/' + src + '" ondragstart="return false"/>';
...
function removeImg(obj)
{
	...
	var img = parent.find('img').attr('src').replace('/upload/tlib/thumb/', '');
...
function tarlocate()
{
	...
	jQuery('.pack-img img').each(function(){
		var img = jQuery(this).attr('src').replace('/upload/tlib/thumb/', '');
		if (jQuery('input[name=thumb]').val() == '') {
