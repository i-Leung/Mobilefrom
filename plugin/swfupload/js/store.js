...
function uploadStoreProgress(file, bytesLoaded) {
	try {
	    //改变flash接收的成功上传图片数量
		var stats = storeswfu.getStats();
		stats.successful_uploads = parseInt(jQuery('#store-thumbnails').find('img').length);
		storeswfu.setStats(stats);
...
function uploadStoreSuccess(file, serverData) {
	try {
		var progress = new FileProgress(file,  this.customSettings.upload_target);

		if (serverData.substring(0, 7) === "FILEID:") {
			addStoreImage(serverData.substring(7));
...
function addStoreImage(src) {
	//添加移除功能用
	var num = jQuery('#store-thumbnails').find('img').length;
	var img = '<img style="margin:5px;border:solid 1px #ddd;" src="/upload/store/thumb/' + src + '" ondragstart="return false"/>';
...
function removeImg(obj)
{
	...
	var img = parent.find('img').attr('src').replace('/upload/store/thumb/', '');
...
function tarlocate()
{
	...
	jQuery('.pack-img img').each(function(){
		var img = jQuery(this).attr('src').replace('/upload/store/thumb/', '');
		if (jQuery('input[name=thumb]').val() == '') {
