		...
		success:function(params){
			if(params != '0'){
				...
				jQuery.each(params.gallery, function(k, v){
					...
					pic = '<img style="margin:5px;border:solid 1px #ddd;" src="' + tabletSrc + 'thumb/' + v + '" onerror="loadErrorImg(this);" />';
					pack = '<div class="pack">\
...
//检测平板状况描述情况
function checkTabletDescription()
{
	checkUsed();
	var isnew = jQuery('input[name=isnew]:checked').val();
	switch (isnew) {
...
function submitTablet(obj)
{
	...
	jQuery.ajax({
		...
		dataType:'json',
		url:'/customer/tablet/publish-post',
		beforeSend:function(){
			jQuery(obj).html('正在发布...');
		},
		success:function(data){
			if (data != '0') {
				window.location.href = '/customer/tablet/success?id=' + data;
			} else {
