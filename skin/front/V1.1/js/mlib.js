				...
				jQuery.each(data, function(k, v){
					...
					var html = '<li ' + style + '>\
									<div class="m_l_i_a_thumb m_l_i_a_item_layout">\
										<a href="/mlib/item?id=' + v.id + '" target="_blank">\
											<img src="' + v.thumb + '" onerror="loadErrorImg(this);" />\
										</a>\
									</div>\
									<div class="m_l_i_a_title m_l_i_a_item_layout">\
										<a href="/mlib/item?id=' + v.id + '" target="_blank" title="' + v.brand + ' ' + v.model + '">' + v.brand + ' ' + v.model + '</a>\
									</div>\
					...
					jQuery('.mlib_list_item_area ul').append(html);
				});
			} else {
				jQuery(obj).hide();
			}
