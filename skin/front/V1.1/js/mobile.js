 ...
 */
//加载可见手机列表矩阵展示样式
function loadVisibleMobileByGrid()
{
					...
					jQuery.each(data, function(k, v){
						var item = '<li>\
								            <div class="mb_img_name">\
								                <a target="_blank" href="/mobile/item?cfrom=list&id=' + v.id + '">\
...
//加载可见手机列表列表展示样式
function loadVisibleMobileByList()
{
	if((jQuery('.list_buttom').css('display') == 'none') && (jQuery('.loading').css('display') == 'none')){
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				if(data == '0'){
					...
					jQuery.each(data, function(k, v){
						var item = '<li>\
										<a target="_blank" class="pro_img" title="' + v.title + '" href="/mobile/item?cfrom=list&id=' + v.id + '">\
											<img title="' + v.title + '" alt="' + v.title + '" src="' + baseSrc + 'thumb/' + v.thumb + '" onerror="loadErrorImg(this);" />\
										</a>\
										<div class="pro_info">\
											<a title="' + v.title + '" target="_blank" class="blue" href="/mobile/item?cfrom=list&id=' + v.id + '">' + v.title + '</a>\
...
//加载可见手机列表显示为列形式
function loadSearchMobileByList(q)
{
	if((jQuery('.list_buttom').css('display') == 'none') && (jQuery('.loading').css('display') == 'none')){
		var next = parseInt(jQuery('#view_list').attr('page')) + 1;
		jQuery.ajax({
...
//加载可见手机列表显示为块形式
function loadSearchMobileByGrid(q)
{
	if((jQuery('.list_buttom').css('display') == 'none') && (jQuery('.loading').css('display') == 'none')){
		var next = parseInt(jQuery('#view_grid').attr('page')) + 1;
		jQuery.ajax({
			...
			success:function(data){
				...
				if(data == '0'){
					...
					jQuery.each(data, function(k, v){
						var item = '<li>\
								            <div class="mb_img_name">\
								                <a target="_blank" href="/mobile/item?cfrom=search&id=' + v.id + '">\
								                    <img alt="' + v.title + '" title="' + v.title + '" src="' + baseSrc + 'thumb/' + v.thumb + '" onerror="loadErrorImg(this);" />\
								                    <span title="' + v.title + '">[<span class="red" style="display:inline;">' + v.region + '</span>]' + v.title + '</span>\
								                </a>\
								            </div>\
...
//添加关注
function addMobileFavor(obj, id)
{
	jQuery.ajax({
		type:'get',
		data:{id:id, type:'1'},
		...
		success:function(data){
			if(data == '1') {
				jQuery(obj).html('取消收藏').attr('onclick', 'removeMobileFavor(this, \'' + id + '\')');
			}
		},
		error:function(){
			alert('收藏信息失败，请稍候尝试');
...
//取消关注
function removeMobileFavor(obj, id)
{
	jQuery.ajax({
		type:'get',
		data:{type:'1',id:id},
		...
		success:function(data){
			if(data == '1') {
				jQuery(obj).html('加入收藏').attr('onclick', 'addMobileFavor(this, \'' + id + '\')');
			}else{
				alert('取消收藏失败，请稍候再尝试');
			}
		},
				...
				if(from == 'list'){
					...
					var html = '<li>\
					            		<div class="mobile_img_c">\
					            			<a href="/mobile/item?cfrom=sidebox&id=' + id + '" target="_blank">\
					...
					var html = '<li>\
							            ...
							            <div class="mobile_name_c">\
							            	<a href="/mobile/item?cfrom=sidebox&id=' + id + '" target="_blank">' + title + '</a>\
							            </div>\
							            <i onclick="removeCompare(this, \'' + id + '\', \'side\')"></i>\
							            <div class="clear"></div>\
							        </li>';
...
function removeCompare(obj, id, from)
{
	jQuery.ajax({
		...
		data:{id:id},
		dataType:'json',
		url:'/mobile/list/remove-compare',
		success:function(data){
			if(data == '1'){
				if(from == 'list'){
		...
		success:function(data){
			if(data != '0'){
				...
				jQuery.each(data, function(k,v){
					var html = '<li>\
	            		<div class="mobile_img_c">\
	            			<a href="/mobile/item?cfrom=sidebox&id=' + v.id + '" target="_blank">\
	            				<img alt="' + v.title + '" title="' + v.title + '" src="' + baseSrc + 'thumb/' + v.thumb + '" onerror="loadErrorImg(this);" />\
	            			</a>\
			            </div>\
			            <div class="mobile_name_c">\
			            	<a href="/mobile/item?cfrom=sidebox&id=' + v.id + '" target="_blank">' + v.title + '</a>\
			            </div>\
		...
		success:function(data){
			if(data != '0'){
				jQuery.each(data, function(k, v){
					...
					var wp = '<a href="/mobile/item?cfrom=compare&id=' + v.id + '" target="_blank">\
				                		<img src="' + baseSrc + 'thumb/' + v.thumb + '" onerror="loadErrorImg(this);"/>\
				                    </a>\
				                    <div class="mobile_name">\
				                    	<a href="/mobile/item?cfrom=compare&id=' + v.id + '">' + v.title + '</a>\
				                    </div>\
				                    <a class="guanzhu btn_red" style="display:none;" href="javascript:void(0)" onclick="addMobileFavor(this, ' + v.id + ')">+加入关注</a>';
					...
					jQuery('.to_see:eq(' + k + ')').html('<a href="/mobile/item?cfrom=compare&id=' + v.id + '" target="_blank" class="btn_red wanted">我要这部</a>');
				});
			}
		},
		error:function(){}
...
function showGalleryPic(obj)
{
	...
	jQuery('.img_b a').attr('href', '#' + src.replace('/upload/mobile/thumb/', ''));
				...
				jQuery.each(data, function(k, v){
					...
					html += '<li>\
					                <dl class="zx_user">\
					                    ...
					                    <dd>\
					                    	<div class="user_name_time">\
					                        	<a style="color:#9C9A9C;" target="_blank" href="/space/mobile/list?cfrom=consult&id=' + v.asker + '">' + v.username + '</a>\
...
//获取用户发布的其它手机信息
function loadOtherMobile(customer_id, mobile_id)
{
	jQuery.ajax({
		type:'get',
		data:{cid:customer_id,mid:mobile_id},
		dataType:'json',
		url:'/space/mobile/other',
		success:function(data){
			if(data != '0') {
				var html = '<li>\
				            	<div class="mobile_info">\
				                	<div class="mb_img_s">\
				                    	<a href="/mobile/item?cfrom=item_other&id=' + data.id + '">\
			...
			if(data != 0){
				jQuery.each(data, function(k, v){
					var html = '<li>\
									<div class="mobile_info">\
										<div class="mb_img_s">\
											<a href="/mobile/item?cfrom=history&id=' + v.id + '">\
												<img src="' + baseSrc + 'thumb/' + v.thumb + '" onerror="loadErrorImg(this);"/>\
											</a>\
										</div>\
										<div class="mb_name_price">\
											<div class="mb_name">\
												<a href="/mobile/item?cfrom=history&id=' + v.id + '">' + v.title+ '</a>\
											</div>\
...
//清空用户浏览记录
function clearHistoryMobile()
{
	jQuery.ajax({
		type:'get',
		dataType:'json',
		url:'/mobile/list/clear-history',
		success:function(data){
			if(data == '1'){
				jQuery('#history').hide();
			}else{
